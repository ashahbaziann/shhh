...
void	handle_special_chars(t_shell **shell, char *current)
{
	...
	while (*current)
	{
		...
		if (*current == '|')
			add_token(&shell, TOKEN_PIPE, state, extract_separator(&(current)));
		else if (*current == '<' && (*(current + 1)) == '<')
			add_token(&shell, TOKEN_REDIR_HEREDOC, state, extract_separator(&(current)));
		else if (*current == '>' && (*(current + 1)) == '>')
			add_token(&shell, TOKEN_REDIR_APPEND, state, extract_separator(&(current)));
		else if (*current == '<')
			add_token(&shell, TOKEN_REDIR_IN, state, extract_separator(&(current)));
		else if (*current == '>')
			add_token(&shell, TOKEN_REDIR_OUT, state, extract_separator(&(current)));
		else if (*current == '$')
			add_token(&shell, TOKEN_ENV_VAR, state, extract_env_var(&(current),(*shell)));
		else if (!(is_separator(*current)) && !(is_space(*current)))
			add_token(&shell, TOKEN_WORD, state, extract_word(&(current), ((*shell))));
